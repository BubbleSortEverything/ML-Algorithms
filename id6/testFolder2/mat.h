...
//zzz    Matrix &print(std::string msg="") const;         // print matrix and its name (returns arg for pipes)
    void print(std::string msg="") const;         // print matrix and its name (returns arg for pipes)
    ...
    void printLabeledRow(SymbolNumMap &labels, std::string msg="") const;   // print matrix with rows labeled
    void printStrings(const SymbolNumMap &labels, std::string msg="") const;   // print matrix containing only strings
    ...
    SymbolNumMap readLabeledRow();       // read in a matrix plus row labels
    SymbolNumMap readStrings();           // read in a matrix which contains all strings
    SymbolNumMap readStrings(SymbolNumMap &syms);   // read in a matrix which contains all strings updating symbol map

protected:
    SymbolNumMap readAux(ElementType labeled, bool transpose);    // general read routine both labeled and un
...
protected:
    ...
    void qsCol(int c, int lower, int upper);          // quick sort
...
public:
    ...
    Matrix subMatrixPickRows2(int match, const Matrix &list);  // pick rows i in self for which list[i]==match
