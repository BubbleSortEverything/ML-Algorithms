...
//  class SymbolNumMap
//
//  a class for a 1-1 mapping std::string to int.   Useful if you want to read in 
//  data as strings and force them into a numeric matrix.
...
//zzz Matrix &Matrix::print(std::string msg) const
...
void Matrix::printLabeledRow(SymbolNumMap &labels, std::string msg) const
{
...
void Matrix::printStrings(const SymbolNumMap &symbols, std::string msg) const
{
...
SymbolNumMap Matrix::readLabeledRow()
{
    return readAux(LABELEDROW, false);
}


// read in a matrix which contains all strings
SymbolNumMap Matrix::readStrings()
{
    return readAux(STRINGS, false);
}
...
SymbolNumMap Matrix::readAux(ElementType labeled, bool transpose)
{
    SymbolNumMap syms("", 1000);

    return readAux(labeled, transpose, syms);
}
...
// deallocate the other matrix!!   In a sense this is not a real matrix.
// If you want this matrix to persist then you have to make a full copy of it.
Matrix Matrix::subMatrixPickRows2(int match, const Matrix &list)
{
    /*assertDefined("lhs of subMatrixPickRows");
    list.assertDefined("rhs of subMatrixPickRows");
    list.assertColVector("rhs of subMatrixPickRows");
