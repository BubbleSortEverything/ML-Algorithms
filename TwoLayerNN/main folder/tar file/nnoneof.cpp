 ...
 *  Professor: Dr. Rober Baker Heckendorn
...
int main(int argc, char const *argv[])
{
    ...
    //trainingMatrixWithBias.print("trainingMatrixWithBias");
   ...
   /*   Matrix for Maxa arg in the each row of the result
    */
    Matrix argMax = result.argMaxRow();
    Matrix expectedOutput = testMatrix.extract(0, numOfInputs, testMatrix.numRows(), 1);
    expectedOutput.printfmt("Target");
    argMax.printfmt("Predicted");

    for(int i = 0; i < testTarget.numCols(); i++){
        Matrix confusionMatrix = new Matrix(numOfClasses, numOfClasses, "Confusion Matrix");
        confusionMatrix.constant(0);
        for(int j = 0; j < testTarget.numRows(); j++){
            if(testTarget.get(j, i) == 0){
                if(argMax.get(j, i) == 0){
                    confusionMatrix.inc(0, 0);
                }else if(argMax.get(j, i) == 1){
                    confusionMatrix.inc(1, 0);
                }else{
                    confusionMatrix.inc(2, 0);
                }
            }
            else if(testTarget.get(j, i) == 1){
                if(argMax.get(j, i) == 0){
                    confusionMatrix.inc(1, 1);
                }else if(argMax.get(j, i) == 1){
                    confusionMatrix.inc(1, 1);
                }else{
                    confusionMatrix.inc(2, 1);
                }
            }else if(testTarget.get(j, i) == 2){
                if(argMax.get(j, i) == 0){
                    confusionMatrix.inc(0, 2);
                }else if(argMax.get(j, i) == 1){
                    confusionMatrix.inc(1, 2);
                }else{
                    confusionMatrix.inc(2, 2);
                }
            }
        }
        confusionMatrix.printfmt("Confusion Matrix");
    }
