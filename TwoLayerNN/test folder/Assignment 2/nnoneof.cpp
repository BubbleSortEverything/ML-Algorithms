...
int main(int argc, char ** argv){    
    ...
    //freopen("newIrisData2.in","r", stdin);    // Used for debugging for getting stdin from file
    ...
    // Create new matrix for max arg in each row of output and target
    Matrix argMax = output.argMaxRow();
    Matrix test_argMax = test_target.argMaxRow();
    // Print target and predicted
    test_argMax.printfmt("Target", "", false);
    argMax.printfmt("Predicted","", false);
        ...
        for(int j = 0; j < test_target.numRows(); j++)
        {
            if (test_target.get(j,i) == 0) {
                if (argMax.get(j,i) == 0) {
                    ...
                    confusion_Matrix.inc(2,0);
                }
            }
            else if (test_target.get(j,i) == 1)
            {
                if (argMax.get(j,i) == 0) {
                    ...
                    confusion_Matrix.inc(2,1);
                }
            }
            else if (test_target.get(j,i) == 2)
            {
                if (argMax.get(j,i) == 0) {
                    confusion_Matrix.inc(0,2);
                }
                else if(argMax.get(j,i) == 1) {
                    confusion_Matrix.inc(1,2);
                }
                else{
                    confusion_Matrix.inc(2,2);
                }
            }
